...
var ajax = (function() {
    var jsc = (new Date()).getTime(),
        ...
        rnoContent = /^(?:GET|HEAD|DELETE)$/, rnotwhite = /\S/, rbracket = /\[\]$/,
        jsre = /\=\?(&|$)/, rquery = /\?/, rts = /([?&])_=[^&]*/,
        rurl = /^(\w+:)?\/\/([^\/?#]+)/, r20 = /%20/g, rhash = /#.*$/;
    ...
    var helper = {
        ...
        httpSuccess: function(xhr) {
            try {
                ...
                // should be 204 so treat it as success, see #1450
        ...
        httpData: function(xhr, type, s) {
            ...
            if (typeof data === "string") {
                if (type === "json" || !type && ct.indexOf("json") >= 0) {
                    data = data ? (window.JSON && window.JSON.parse 
                            ? window.JSON.parse(data) 
                            : (new Function("return " + data)())) : data;
    ...
    ajax.settings = {
        ...
        async : true,
        /*
         * timeout: 0, data: null, username: null,
         * password: null, traditional: false,
         */
